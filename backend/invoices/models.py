...
class Invoice (models.Model):
    ...
    fac_idclient=models.ForeignKey(Contact, to_field='id', db_column = 'fac_idclient', on_delete = models.SET_NULL, null=True, blank=True)
    #fac_iduser= models.IntegerField(null = True, default=0)
    fac_iduser=models.ForeignKey(User, to_field='id', db_column = 'fac_iduser', on_delete =models.SET_NULL, null=True, blank=True)
    ...
    fac_uuid=models.CharField(max_length=40, null =True,blank=True)
    ...
    def upload(self, filepath):
        ...
        if None==version:
            version=root.get('Version')
        #print(version)
        nsmap['cfdi']='http://www.sat.gob.mx/cfd/'+str(int(float(version)))
        ...
        if Contact.objects.filter(contact_taxid=inv['fac_receptorrfc']).exists():
            ...
            clientlist=Contact.objects.filter(contact_taxid=inv['fac_receptorrfc'])[:1]
            for o in clientlist:
                client=o

            inv['fac_idclient']=client
            if ContactUser.objects.filter(cu_contact_id=client).exists():
                contactusers=ContactUser.objects.filter(cu_contact_id=client)[:1]
                for u in contactusers:
                    cu=u
    ...
    def updateUUID(self, facId):
        print('fac_key en uuid')
        print(facId)
        r = connection.cursor();
        query="""
        update cbr_facturas set fac_uuid=xpath('cfdi:Comprobante/cfdi:Complemento/tfd:TimbreFiscalDigital/@UUID',fac_xml,ARRAY[ARRAY['cfdi','http://www.sat.gob.mx/cfd/4'],ARRAY['tfd','http://www.sat.gob.mx/TimbreFiscalDigital']])  
        """
        query =  query + ' where fac_key = ' + str(facId)
...
class InvoiceLog(models.Model):
    ...
    log_invoice = models.ForeignKey(Invoice, to_field= 'fac_key', db_column = 'log_invoice', on_delete = models.CASCADE)
    log_cuser = models.ForeignKey(User, to_field = 'id', db_column = 'log_cuser', on_delete = models.SET_NULL, blank=True, null=True)
...
class InvoiceReminders(models.Model):
    ...
    rem_invoice = models.ForeignKey(Invoice, to_field= 'fac_key', db_column = 'rem_invoice', on_delete = models.CASCADE) 
    ...
    rem_cuser = models.ForeignKey(User, to_field='id', db_column = 'rem_cuser', on_delete = models.SET_NULL, blank = True, null = True)
...
class InvoicePaymentHistory(models.Model):
    ...
    his_invoice = models.ForeignKey(Invoice, to_field= 'fac_key', db_column = 'his_invoice', on_delete = models.CASCADE) 
...
class InvoiceEdHistory(models.Model):
    ...
    ed_invoice = models.ForeignKey(Invoice, to_field= 'fac_key', db_column = 'his_invoice', on_delete = models.CASCADE) 
    ...
    ed_user = models.ForeignKey(User, to_field='id', db_column = 'ed_user', on_delete = models.SET_NULL , blank = True, null = True)
...
class InvoiceFileLog(models.Model):
    ...
    ifl_invoice=models.ForeignKey(Invoice, to_field='fac_key', db_column = 'ifl_invoice', on_delete = models.SET_NULL, null=True, blank=True)
